...
namespace input_output
{
    ...
    void WriteDIBHeader(unsigned char *dib_header, size_t width, size_t height)
    {
        ...
        bmp24::NumToBytes(width * height * bmp24::utils::BYTES_PER_PIXEL, dib_header + bmp24::utils::IMAGE_DATA_SIZE_POSITION);
    }
    ...
    void WriterBMP24::Write(const ImageBMP &image_bmp)
    {
        ...
        bmp24::BMP24DIB_Header bmp24_dib_header{image_bmp.GetWidth(), image_bmp.GetHeight()};
        ...
        img_file.write(reinterpret_cast<char *>(bmp24_dib_header.GetBMP24DIB_HeaderBytes()), bmp24::utils::DIB_HEADER_SIZE);
        ...
        for (size_t i = 0; i < image_bmp.GetHeight(); ++i)
        {
            ...
            {
                uint8_t pix[] = {image_bmp.GetImagePixel(i, j).GetBlue(), image_bmp.GetImagePixel(i, j).GetGreen(), image_bmp.GetImagePixel(i, j).GetRed()};
