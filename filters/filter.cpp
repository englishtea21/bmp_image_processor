...
namespace filters
{
    ...
    Pixel<uint8_t> Grayscale::GetPixel(const ImageBMP &image, size_t y, size_t x) const
    {
        ...
        tmp_pixel.SetPixel(filters::utils::matrices::GRAYSCALE_COLOR_RATIO[0] *
                               static_cast<double>(tmp_pixel.GetBlue()) +
                           filters::utils::matrices::GRAYSCALE_COLOR_RATIO[1] *
                               static_cast<double>(tmp_pixel.GetGreen()) +
                           filters::utils::matrices::GRAYSCALE_COLOR_RATIO[2] *
    ...
    ImageBMP EdgeDetection::Apply(const ImageBMP &image)
    {
        ...
        for (size_t i = 0; i < image_tmp.GetHeight(); ++i)
        {
            ...
            {
                image_tmp.SetPixel(i, j, image_tmp.GetImagePixel(i, j).GetBlue() > this->threshold_ ? filters::utils::pixels::WHITE : filters::utils::pixels::BLACK);
            }
        }
        return image_tmp;
    }
    ...
    std::vector<std::vector<double>> GaussianBlur::Generate1DGaussianKernel() const
    {
        ...
        {
            gaussian_kernel_1d[0][i] = 1 / std::exp(std::pow(this->kernel_optimal_size_ / 2 - i, 2) / (2 * this->sigma_square_));
    ...
    ImageBMP Pixelization::Apply(const ImageBMP &image)
    {
        ...
        for (size_t i = 0; i < image.GetHeight(); i += window_size_)
        {
            ...
            {
                std::fill(row.begin() + j, row.begin() + std::min(j + window_size_, image.GetWidth()), GetPixel(image, i, j));
            }
            // new_data[i] = std::move(row);
    ...
    Pixel<uint8_t> Pixelization::GetPixel(const ImageBMP &image, size_t y, size_t x) const
    {
        ...
        for (size_t i = std::clamp(static_cast<int64_t>(y) - static_cast<int64_t>(window_size_) / 2, static_cast<int64_t>(0), static_cast<int64_t>(image.GetHeight()) - 1); i < std::clamp(y + window_size_ / 2, static_cast<size_t>(0), image.GetHeight() - 1); i++)
        {
        ...
        // red
        std::nth_element(pixel_candidates.begin(), median_it, pixel_candidates.end(), [](auto lhs, auto rhs)
    ...
    void Posterization::MakeColorsQuantization(const std::vector<uint8_t> &min_colors, const std::vector<uint8_t> &max_colors)
    {
    ...
    {
        auto lower = std::lower_bound(colors_quantization_[color_num].begin(), colors_quantization_[color_num].end(), channel_color);
    ...
    Pixel<uint8_t> Posterization::GetPixel(const ImageBMP &image, size_t y, size_t x) const
    {
        ...
        return {QuantizeColor(tmp_pixel.GetBlue(), 0), QuantizeColor(tmp_pixel.GetGreen(), 1), QuantizeColor(tmp_pixel.GetRed(), 2)};
    }
    ...
    ImageBMP Posterization::Apply(const ImageBMP &image)
    {
        ...
        {
            // min color
            min_colors[0] = std::min(std::min_element(image.GetImagePixels()[i].begin(), image.GetImagePixels()[i].end(), [](auto lhs, auto rhs)
                                                      { return lhs.GetBlue() < rhs.GetBlue(); })
                                         ->GetBlue(),
                                     min_colors[0]);
            min_colors[1] = std::min(std::min_element(image.GetImagePixels()[i].begin(), image.GetImagePixels()[i].end(), [](auto lhs, auto rhs)
                                                      { return lhs.GetGreen() < rhs.GetGreen(); })
                                         ->GetGreen(),
                                     min_colors[1]);
            min_colors[2] = std::min(std::min_element(image.GetImagePixels()[i].begin(), image.GetImagePixels()[i].end(), [](auto lhs, auto rhs)
                                                      { return lhs.GetRed() < rhs.GetRed(); })
                                         ->GetRed(),
                                     min_colors[2]);

            // max colors
            max_colors[0] = std::max(std::max_element(image.GetImagePixels()[i].begin(), image.GetImagePixels()[i].end(), [](auto lhs, auto rhs)
                                                      { return lhs.GetBlue() < rhs.GetBlue(); })
                                         ->GetBlue(),
                                     min_colors[0]);
            max_colors[1] = std::max(std::max_element(image.GetImagePixels()[i].begin(), image.GetImagePixels()[i].end(), [](auto lhs, auto rhs)
                                                      { return lhs.GetGreen() < rhs.GetGreen(); })
                                         ->GetGreen(),
                                     min_colors[1]);
            max_colors[2] = std::max(std::max_element(image.GetImagePixels()[i].begin(), image.GetImagePixels()[i].end(), [](auto lhs, auto rhs)
                                                      { return lhs.GetRed() < rhs.GetRed(); })
                                         ->GetRed(),
                                     min_colors[0]);
        }
