...
class Pixel
{
    static_assert((std::is_same<double, T>() || std::is_same<int64_t, T>() || std::is_same<uint8_t, T>()), "Only double, int64_t, uint8_T types allowed!");
...
template <typename T>
template <typename U>
Pixel<T>::Pixel(const Pixel<U> &pixel) : Pixel<T>(static_cast<T>(pixel.GetBlue()), static_cast<T>(pixel.GetGreen()), static_cast<T>(pixel.GetRed()))
{
}
...
Pixel<uint8_t> Pixel<T>::NormalizePixel(const uint8_t max_color) const
{
    Pixel<uint8_t> normalized_pixel{
        static_cast<uint8_t>(std::clamp(static_cast<int64_t>(blue_), static_cast<int64_t>(0), static_cast<int64_t>(max_color))),
        static_cast<uint8_t>(std::clamp(static_cast<int64_t>(green_), static_cast<int64_t>(0), static_cast<int64_t>(max_color))),
        static_cast<uint8_t>(std::clamp(static_cast<int64_t>(red_), static_cast<int64_t>(0), static_cast<int64_t>(max_color)))};
